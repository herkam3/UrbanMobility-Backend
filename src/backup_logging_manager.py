    ...
    def view_logs(self, limit=50, show_suspicious_only=False):
        ...
        try:
            ...
            if show_suspicious_only and logs:
                suspicious_ids = [log[0] for log in logs if log[6] == 1]  # suspicious = 1
                if suspicious_ids:
        ...
        try:
            ...
            for log in all_logs:
                ...
                if (search_lower in username.lower() or 
                    ...
                    search_lower in additional_info.lower()):
...
class BackupManager:
    ...
    def restore_backup(self, backup_filename, restore_code=None):
        ...
        if current_role == 'super_admin':
            ...
            code_validation = self._validate_restore_code(restore_code, backup_filename)
        ...
        try:
            ...
            # Restore database
            restored_db_path = os.path.join(restore_dir, os.path.basename(self.db.db_path))
